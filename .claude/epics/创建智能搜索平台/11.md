...
priority: high
assignee: ""
labels: ["向量处理", "AI", "智能模式切换"]
created: 2025-09-20T02:23:58Z
updated: 2025-09-20T07:24:00Z
completed: 2025-09-20T07:24:00Z
...
构建智能搜索平台的核心向量处理引擎，支持离线批量处理和在线实时处理两种模式。集成多种向量化API（OpenAI、本地模型等），通过配置文件设置处理模式，为文档和查询提供高质量的向量表示。

## 验收标准

- [x] 离线批量向量化处理，支持大文件和批量文档
- [x] 在线实时向量化，查询响应时间<2秒
- [x] 支持多种向量化模型（OpenAI text-embedding-3-small/large, 本地BERT等）
- [x] 配置化模式切换：通过配置文件设置使用离线或在线向量处理模式
- [x] 向量缓存机制，避免重复计算
- [x] 向量质量评估和监控
- [x] 异步任务队列，支持大规模文档处理
- [x] 向量处理失败重试和错误恢复机制
...
public class VectorProcessingConfig {
    ...
    private ProcessingMode defaultMode = ProcessingMode.ONLINE_REALTIME;
...
```yaml
vector-processing:
  # 配置处理模式：OFFLINE_BATCH 或 ONLINE_REALTIME
  default-mode: ONLINE_REALTIME
...
public class VectorProcessingEngineImpl implements VectorProcessingEngine {
    ...
    @Autowired
    private VectorProcessingConfig processingConfig;
